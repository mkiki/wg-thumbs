...
function generateThumbnail(source, destination, thumb, callback) { 
  ...
  var command = '';
  command += ' ' + CONVERT;
  command += ' "' + Utils.escapeFilenameForCommand(source, true) + '[0]"';
  ...
  command += ' "' + Utils.escapeFilenameForCommand(destination, true) + '"';
...
function makeGIF(sourceFiles, destination, delay, dateTime, callback) {
  ...
  for (var i=0; i<sourceFiles.length; i++)
    command += ' "' + Utils.escapeFilenameForCommand(sourceFiles[i], true) + '[0]"';
  command += ' -loop 0';
  command += ' "' + Utils.escapeFilenameForCommand(destination, true) + '"';
...
function dominantColor(longFilenmae, callback) {
  ...
  var command = '';
  command += ' ' + CONVERT;
  command += ' "' + Utils.escapeFilenameForCommand(longFilenmae, true) + '[0]"';
  command += " -scale 1x1\\!";
